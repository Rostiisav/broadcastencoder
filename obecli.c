    ...
    for (int i = 0; i < cli.program.num_streams; i++)
    {
        ...
        if (stream->stream_type == STREAM_TYPE_VIDEO)
        {
            ...
            for (int j = 0; j < stream->num_frame_data; j++)
            {
                ...
                printf("                     %s:   %s\n", stream->frame_data[j].source == MISC_WSS ? "WSS (converted)" : stream->frame_data[j].source == VBI_RAW ? "VBI" : stream->frame_data[j].source == VBI_VIDEO_INDEX ? "VII" : "VANC", format_name);
            }
        }
...
int run_cmd_prompt(char *history_filename, char *prompt)
{
    while (1)
    {
        if (line_read)
...
int main(int argc, char **argv)
{
    ...
    {
        return run_cmd_prompt(history_filename, prompt);
    }
